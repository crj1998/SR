...
class RSTB(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.patch_embed = PatchEmbed(img_size, patch_size, dim, dim, norm_layer=None)

        self.patch_unembed = PatchUnEmbed(img_size, patch_size, dim)
...
class SwinIR(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans=3, num_feat = 64,
                 ...
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True, upscale=2, 
                 **kwargs):
        ...
        self.upsample = nn.Sequential(
            ...
            nn.Conv2d(num_feat, in_chans, 3, 1, 1)
        )
...
if __name__ == '__main__':
    ...
    # model = SwinIR(
    #     upscale=upscale, img_size=(height, width), patch_size=1,
    #     window_size=window_size, depths=[2, 2, 2, 2],
    #     embed_dim=20, num_heads=[2, 2, 2, 2], mlp_ratio=2
    # )
    model = SwinIR(
        upscale=upscale, img_size=(height, width), patch_size=1, num_feat=32,
        window_size=window_size, depths=[2, 2, 2, 2], embed_dim=16, num_heads=[2, 2, 2, 2], mlp_ratio=2
    )
