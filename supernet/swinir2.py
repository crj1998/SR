    ...
    def __init__(self, embed_dim, resolution, patch_size, depth=2, num_heads=4, window_size=4, mlp_ratio=2, qkv_bias=False, qk_scale=None, 
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                dim=embed_dim, input_resolution=resolution,
        ...
        self.patch_embed = PatchEmbed(resolution, patch_size, embed_dim, embed_dim, norm_layer=None)
...
class SwinIR(nn.Module):
    def __init__(self, upscale=2, img_size=64, window_size=7, patch_size=1, 
                num_feat = 64, embed_dim=96, depths=[6, 6, 6, 6], num_heads=[6, 6, 6, 6],
                 mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0.,
                 norm_layer=nn.LayerNorm, patch_norm=True, in_chans=3, 
                 **kwargs):
        ...
        self.sampled_layers = len(depths)

        self.search_space = {"layers": list(range(self.num_layers+1))}

        ################################### 1, shallow feature extraction ###################################
        self.shallow_feature_extractor = nn.Conv2d(in_chans, embed_dim, 3, 1, 1)

        ################################### 2, deep feature extraction ######################################
        # split image into non-overlapping patches
        self.patch_embed = PatchEmbed(img_size, patch_size, embed_dim, embed_dim, norm_layer if patch_norm else None)
        ...
        self.upsample = nn.Sequential(OrderedDict([
            ("conv1", nn.Conv2d(embed_dim, num_feat, 3, 1, 1)),
            ("act", nn.LeakyReLU(inplace=True)),
            ("conv2", nn.Conv2d(num_feat, num_feat * upscale**2, 3, 1, 1)),
            ("pixelshuffle", nn.PixelShuffle(upscale)),
            ("conv3", nn.Conv2d(num_feat, in_chans, 3, 1, 1))
        ]))
    ...
    def set_sample_config(self, layers):
        self.sampled_layers = min(layers, self.num_layers)

    def sample(self, t="max"):
        if t == "max":
            config = {k: max(v) for k, v in self.search_space.items()}
        elif t == "min":
            config = {k: min(v) for k, v in self.search_space.items()}
        else:
